...
//控制全屏和播放暂停按钮
@property (nonatomic,strong) UIButton  *playOrPauseBtn,*lockBtn,*backBtn,*rateBtn,*voiceBtn;
//进度滑块&声音滑块
...
-(void)initWMPlayer{
    ...
    self.voiceBtn.showsTouchWhenHighlighted = YES;
    [self.voiceBtn addTarget:self action:@selector(voiceAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.voiceBtn setImage:[UIImage imageNamed:@"wmplayer声音开"] forState:UIControlStateNormal];
    [self.voiceBtn setImage:[UIImage imageNamed:@"wmplayer声音关"] forState:UIControlStateSelected];
...
-(void)addUIControlConstraints{
    ...
    [self.rightTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.trailing.equalTo(self.bottomView).offset(-90);
        make.top.equalTo(self.bottomView.mas_centerY).with.offset(8);
    }];
...
- (void)voiceAction:(UIButton *)sender{
    ...
    [self setMuted:_voiceBtn.selected];
}
//设置静音
